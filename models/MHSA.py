...
class Relative_Attention(nn.Module):
    def __init__(
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(
                (2 * img_size[0] - 1) * (2 * img_size[1] - 1) + 1,
                num_heads
            )
        )
        ...
        # Keep historical behaviour for current checkpoint
    ...
    def forward(self, x):
        ...
        expected_N = self.img_size[0] * self.img_size[1] + self.extra_token_num
        if N != expected_N:
            raise RuntimeError(
                "\n"
                "[Relative_Attention token mismatch]\n"
                f"Actual sequence length : {N}\n"
                f"Expected sequence      : {expected_N}\n"
                f"Image grid             : {self.img_size[0]} x {self.img_size[1]}\n"
                f"Spatial tokens         : {self.img_size[0] * self.img_size[1]}\n"
                f"Extra tokens           : {self.extra_token_num}\n"
                "\n"
                "Expected:\n"
                "N = H * W + extra_token_num\n"
                "\n"
                "Please check:\n"
                "CLS/category tokens + meta tokens + part tokens.\n"
            )
        ...
        # Relative position bias
        seq_len = self.img_size[0] * self.img_size[1] + self.extra_token_num
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(seq_len, seq_len, -1)
...
class MHSABlock(nn.Module):
    def __init__(self, input_dim, output_dim,image_size, stride, num_heads,extra_token_num=1,mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,attn_drop=0.,drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        if stride != 1:
