...
class SemanticPartTokenGeneratorV2(nn.Module): # V2
    def __init__(
        self,
        in_dim: int,
        embed_dim: int,
        ...
        # [FIX-1] 半开热启动：保留 tanh/sigmoid 有界性，但从"开"的位置起步，
        # 而不是从饱和区（-5 / 0）等优化器来打开（实测打不开）。
        # curv_sem_alpha 取 0.5（tanh≈0.46）：语义注入开强一点，学生头能更好
        # 拟合 HVP 教师，curv_reg_loss 才不会一直空转（导师建议值）。
    ...
    def forward(self, x, extra_tokens, return_aux: bool = False):
        ...
        if (
            ...
            # [FIX-2] 同时兼容 Apex 与原生 torch autocast，回归分支强制 fp32
            def _run_reg_head():
