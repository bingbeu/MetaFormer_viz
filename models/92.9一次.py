...
class SemanticPartTokenGenerator(nn.Module):
    def __init__(
        self,
        in_dim: int,
        embed_dim: int,
        ...
        # Zero-initialized residual gates preserve the stable semantic part-token
        # baseline at initialization. Classification gradients open only useful
        # curvature/similarity paths instead of injecting noisy priors at epoch 0.
        self.curv_feat_alpha = nn.Parameter(torch.tensor(0.0))
        self.curv_logit_alpha = nn.Parameter(torch.tensor(0.0))
        self.sim_logit_alpha = nn.Parameter(torch.tensor(0.0))
    ...
    def _part_attribute_semantics(self, attr_tokens, batch_size):
        ...
        attr_scale = self.attr_logit_alpha.clamp(0.1, 20.0)
    ...
    def _compute_hvp_curvature(self, x, sem_per_part):
        # HVP is a float32 training-only teacher. The predictor below is used
        # by both training and inference, so the forward behavior is consistent.
    ...
    def forward(self, x, extra_tokens, return_aux: bool = False):
        ...
        # The class token is not an attribute. When no attribute is supplied,
        # it is retained only as a compatibility fallback for image-only runs.
        ...
        sem_summary = sem_per_part.mean(dim=1, keepdim=True).expand_as(x)
        pred_curvature = self.curv_head(x + sem_summary).clamp_min(self.eps)
        ...
        # Stop-gradient prevents the weighting network from reducing the loss by
        # moving mass to already aligned tokens; alignment still updates x/semantics.
        ...
        part_tokens = self.out_norm(
            self.out_proj(part_tokens) + self.part_queries.expand(B, -1, -1)
        )
