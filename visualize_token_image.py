...
用法：
  python visualize_token_image.py --cfg /raid/MetaFormer-V8-Lite/output/MetaFG_meta_2/inat2018_meta/config.json --ckpt /raid/MetaFormer-V8-Lite/output/MetaFG_meta_2/inat2018_meta/best.pth  --out ./figs/inat2018 --num-images 20 --layer 2
...
CUDA_VISIBLE_DEVICES=3 python visualize_token.py --cfg /raid/viz/MetaFormer_viz/output/MetaFG_2/stanfordcars-5-cont/config.json --ckpt /raid/viz/MetaFormer_viz/output/MetaFG_2/stanfordcars-5-cont/best.pth  --out ./figs/cars --num-images 50 --layer 2
...
def main():
    ...
    ap.add_argument("--images", default=None,
                    help="显式输入图片：目录/单图/每行一个路径的txt/逗号分隔多路径。设置后忽略 config 的 val 数据集。")
    ap.add_argument("--labels", default=None,
                    help="可选：与 --images 等长的标签清单（每行一个整数类下标）。不提供时用模型预测当伪标签。")
    args = ap.parse_args()
    ...
    if args.images is not None:
        ...
        if args.labels is not None:
            ...
            if len(labels) != len(image_paths):
                raise ValueError("--labels 行数 (%d) 与图片数 (%d) 不一致"
                                 % (len(labels), len(image_paths)))
        loader, image_names = build_explicit_image_loader(image_paths, labels, cfg)
    else:
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        # 显式输入且未给标签：用模型预测当伪标签，保证一阶梯度图与 manifest 正常。
        if args.images is not None and args.labels is None:
