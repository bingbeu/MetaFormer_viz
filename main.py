...
def train_one_epoch_local_data(
    ...
    if hasattr(raw_model, "cur_epoch"):
        raw_model.cur_epoch = epoch
        raw_model.total_epoch = config.TRAIN.EPOCHS
    ...
    for idx, data in enumerate(data_loader):
        ...
        # 保存原始硬标签（loss_route 用，不能受 mixup 影响）
        ...
        # V8-Lite 类别路由损失（真实标签只用于 loss_route，不参与 cat_token 生成）
        if route_logits is not None:
            loss_route = F.cross_entropy(route_logits, hard_targets)
        else:
            loss_route = None
        ...
        if align_loss is not None and curv_reg_loss is not None:
            ...
            total_loss = cls_loss + aux_scale * part_aux_loss
        else:
            total_loss = cls_loss
        ...
        #     # 导师方案：单个 aux 缩放，20 个 epoch 线性升温到 0.1
        #     aux_scale = 0.1 * min(1.0, (epoch + 1) / 20.0)
        #     # part_aux_loss 内部已经是 align_loss + curv_reg_weight * curv_reg_loss
        #     # （模块里 curv_reg_weight 已按导师建议设为 0.05）
        #     total_loss = cls_loss + aux_scale * part_aux_loss
        # else:
        #     total_loss = cls_loss
        ...
        if accum_steps > 1:
            ...
            if should_step:
                ...
                # [ADD] inf/nan 保护：跳过坏步（AMP 下 HVP 偶发溢出）
                if torch.is_tensor(grad_norm) and not torch.isfinite(grad_norm):
                    logger.warning(f"[SKIP] inf/nan grad_norm at epoch {epoch} step {idx}")
                    optimizer.zero_grad()
                    continue
