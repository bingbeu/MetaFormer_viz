...
def hvp_forward_with_internal_aux(model, samples, meta, layer, seed=None):
    """
    One HVP assessment forward that keeps the whole model in eval mode.

    The modified MetaFG/PartGenerator exposes ``force_hvp=True`` specifically
    for visualization/evaluation.  This is safer than toggling part_gen.train(),
    because BN/Dropout/DropPath states stay in inference mode.
    """
    ...
    try:
        ...
        with torch.enable_grad():
            output = model(samples, meta, return_aux=True, force_hvp=True)
        logits, outer_aux, attn1, attn2 = unpack_model_output(output)
        ...
        if inner.get("hvp_curvature") is None:
            raise RuntimeError(
                ...
                "Please confirm you replaced both SemanticPartTokenGeneratorV6.py "
                "and the outer MetaFG model with the modified versions." % layer
            )
...
def collect_teacher_student_fidelity(model, samples, meta, layer, repeats=1, seed=0):
    """
    Reproduce the *actual* distillation space of the uploaded V6 implementation.

    The training code uses Smooth-L1 between:
        student = normalize(pred_curvature_reg)
        teacher = normalize(hvp_curvature)
    where normalize() is per-image mean normalization.

    Returns [B,N]: teacher_mean, student_mean, teacher_self_spearman.
    """
    ...
    for r in range(repeats):
        ...
        if teacher.ndim != 2 or student.ndim != 2 or teacher.shape != student.shape:
            raise RuntimeError("student/teacher shape mismatch: %s vs %s" % (student.shape, teacher.shape))
        teacher_runs.append(teacher)
        student_runs.append(student)

    T = np.stack(teacher_runs, axis=0)  # R,B,N
    S = np.stack(student_runs, axis=0)
...
def _shared_positive_display(a, b, low=1.0, high=99.0):
    a = np.asarray(a, dtype=np.float32).reshape(-1)
    b = np.asarray(b, dtype=np.float32).reshape(-1)
    both = np.concatenate([a, b], axis=0)
    lo = float(np.percentile(both, low))
    hi = float(np.percentile(both, high))
    if not np.isfinite(lo):
        lo = float(np.nanmin(both))
    if not np.isfinite(hi) or hi <= lo + 1e-12:
        hi = lo + 1e-12
    aa = np.clip((a - lo) / (hi - lo), 0.0, 1.0)
    bb = np.clip((b - lo) / (hi - lo), 0.0, 1.0)
    return aa, bb


def plot_distillation_1x4(img, teacher, student, stem, topk=8, alpha=0.55,
    ...
    td, sd = _shared_positive_display(t, s)
    tg = token_grid(td)
    sg = token_grid(sd)
    diff = robust_norm(token_grid(np.abs(t - s)))
    ...
    axes[2].imshow(resize_grid(sg, h, w), cmap=HEAT_CMAP, vmin=0, vmax=1,
                   alpha=alpha, interpolation="bilinear")
    clean_axis(axes[2], "Student curvature\nmean-normalized")
...
def plot_part_generation_1x4(img, curvature, token_semantic, attn_raw, stem, alpha=0.55):
    """Visualize the *actual uploaded-model path* without inventing r17 saliency.

    Current architecture:
      hierarchical semantic query + token_part_sim
                 + curvature-modulated features/logits
                         -> softmax(attn_logits)=attn_raw
                         -> part_tokens = dropout(attn_raw) @ v
    """
    curv = _flatten_score(curvature)
    sem = _flatten_score(token_semantic)
    A = np.asarray(attn_raw, dtype=np.float32)
    ...
    # Since each row of attn_raw is already a probability distribution over N,
    # use the mean attention across parts as a clean union/coverage diagnostic.
    union = A.mean(axis=0)
    ...
    show_heat(axes[2], img, sem, "Token-part semantic similarity\nwhat-to-look-for",
              alpha=max(alpha, 0.60), cmap=HEAT_CMAP)
    show_heat(axes[3], img, union, "Mean part-token attention\n+ part centers", alpha=alpha)
    ...
    for q, (rr, cc) in enumerate(centers):
        ...
        axes[3].text(x + 2, y - 2, "P%d" % (q + 1), color="red", fontsize=7, fontweight="bold")

    fig.suptitle("From Curvature and Semantics to Part-Token Generation", fontsize=13, fontweight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.93])
    save_fig(fig, stem)
...
def main():
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if not args.no_teacher:
            try:
                teacher_batch, student_train_batch, teacher_rel_batch = collect_teacher_student_fidelity(
        ...
        for i in range(B):
            ...
            if teacher_batch is not None and student_curvature_eval is not None:
                teacher_curv = teacher_batch[i]
                student_eval_curv = student_curvature_eval
                teacher_self = (
                ...
                dm = plot_distillation_1x4(
                    img, teacher_curv, student_eval_curv,
                ...
                if student_train_batch is not None:
                    dm["train_spearman"] = spearman_np(teacher_curv, student_train_batch[i])
                    dm["train_topk_overlap"] = topk_overlap(teacher_curv, student_train_batch[i], k)
                else:
                ...
                fig_status["02"] = "SKIP: HVP teacher unavailable; inspect batch warning above"
            ...
            if attn_raw is not None and attn_raw.ndim == 2:
                ...
                if per_token_sim is not None:
                    plot_part_generation_1x4(
                        img, curvature, per_token_sim, attn_raw,
    ...
    save_csv(
        ...
        ["image", "layer", "topk", "pearson", "spearman", "topk_overlap", "mae",
    ...
    print("  student = mean-normalized predicted curvature")
    print("  teacher = mean-normalized HVP curvature")
